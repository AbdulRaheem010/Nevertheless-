...
const firebaseConfig = {
  ...
  function studentLoginAction() {
      ...
      if (!student) {
          alert('Student not registered. Please contact administrator to register you in the system.');
          return;
      }
      
      currentState.studentId = studentId;
      currentState.studentName = studentName;
      currentState.selectedClass = student.class;
      currentState.selectedArm = student.arm;
      
      studentLogin.style.display = 'none';
      classSelection.style.display = 'block';
      studentInfo.textContent = `${studentName} (${studentId})`;
      
      // Highlight the student's class
      document.querySelectorAll('.class-select').forEach(btn => {
          btn.classList.remove('active');
          if (btn.getAttribute('data-level') === student.class) {
              btn.classList.add('active');
              showArmsSelection(student.class);
              
              // Auto-select the student's arm
              setTimeout(() => {
                  const armBtn = document.querySelector(`#arms-container button[data-arm="${student.arm}"]`);
                  if (armBtn) {
                  ...
                  <div class="footer">
                      <div class="signature-line">
                          ${currentSignatures.coordinator ? 
                              `<img src="${currentSignatures.coordinator}" alt="Exam Coordinator Signature" class="signature-image">` : 
                              '<div class="signature-placeholder"></div>'
                          }
                          <div class="signature-name">Exam Coordinator</div>
                      </div>
                      <div class="signature-line">
